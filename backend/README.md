...
| DB Web | `http://127.0.0.1:8080` (dev에서 sqlite-web 자동 기동) |
...
| SSH | Paramiko |
| Excel | openpyxl |
| Config | pydantic-settings (`backend.dev.env` / `backend.prod.env` 로딩) |
| DB 클라이언트 | sqlite-web (dev 모드 한정, 8080 포트) |
...
Docker 실행 시 `backend.dev.env` 또는 `backend.prod.env`가 컨테이너 env로 주입됩니다.
실제 env 파일은 git에 포함되지 않으며, 템플릿은 `deploy/env/backend.dev.env.example`,
`deploy/env/backend.prod.env.example` 입니다. 주요 항목:
...
| `BACKEND_PORT` | `10223` | uvicorn 포트 |
| `SQLITE_WEB_ENABLED` | `1` (dev) / `0` (prod) | sqlite-web 자동 기동 여부 |
| `SQLITE_WEB_HOST` | `0.0.0.0` | sqlite-web bind host |
| `SQLITE_WEB_PORT` | `8080` | sqlite-web 포트 |
| `SQLITE_WEB_READ_ONLY` | `0` | `1`이면 read-only 모드 |
...
../deploy/run-dev.sh  # 기동 시 init_db + seed_admin 다시 실행됨
```
...
`requirements.txt` 에 `sqlite-web==0.7.2` 가 포함되어 있고, `Dockerfile` 이
`8080/tcp` 를 EXPOSE 합니다. `deploy/run-dev.sh` 가 컨테이너 안에서
`sqlite_web` 프로세스를 백그라운드로 함께 띄웁니다.

```bash
# 개발 중 DB 직접 조회
open http://127.0.0.1:8080         # 또는 브라우저로 접속
```

운영 모드(`run-prod.sh`)는 기본 OFF 입니다. 필요한 경우에만
`backend.prod.env` 에서 `SQLITE_WEB_ENABLED=1` 로 켭니다 (이때는 보안상
`SQLITE_WEB_READ_ONLY=1` 권장).
