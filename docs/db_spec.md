...
> RTD Rule/Macro 목록 및 버전 정보는 파일 시스템/외부 연동으로 조회하며 별도 테이블을 두지 않는다. 컴파일/테스트 상태는 프론트/백엔드 세션 메모리에 유지되고, RTD 테스트 시작 시 생성되는 `task_id` 기준으로 완료된 테스트만 `test_results`에 기록된다.
